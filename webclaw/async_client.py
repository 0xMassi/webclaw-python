...
class AsyncWebclaw:
    ...
    async def map(self, url: str, *, search: str | None = None, limit: int | None = None, cursor: str | None = None) -> MapResponse:
        """Discover URLs from a site's sitemap."""
        return ep.parse_map(await self._request("POST", "/v1/map", json=ep.build_map_body(url, search=search, limit=limit, cursor=cursor)))
    ...
    async def search(
        ...
        *,
        ...
        """Async mirror of :meth:`Webclaw.search`.

        Source filters are strict. Freshness and publication-date fields are
        provider discovery hints; ``published_before`` is exclusive.
        """
    ...
    async def watch_get(self, watch_id: str) -> WatchDetail:
        """Get a single watch monitor by ID."""
        return ep.parse_watch_detail(await self._request("GET", f"/v1/watch/{ep.path_segment(watch_id)}"))
